...
def all_vowel_pairs(words)
  sets = []
  vowels = ["a", "e", "i", "o", "u"]
  words.each_with_index do |first_word,idx|
    words.each_with_index do |second_word, idx2|
      set = first_word + " " + second_word
      if idx < idx2 && vowels.all? { |letter| set.include?(letter) }
        sets << set
      end
    end
  end
...
def find_bigrams(str, bigrams)
  bigrams.reject { |ele| !str.include?(ele) }
end
...
class Hash
  def my_select(&prc)
    ...
    nh = {}
    self.each { |key, value| if prc.call(key,value); nh[key]=value; end }
    return nh
  end
end
